        ...
        <div className="flex h-screen w-full" style={{
            ...
            <div className="z-20 w-full flex flex-col items-center justify-center">
                <form onSubmit={handleSubmit} className="md:w-96 min-w-120 flex flex-col items-center justify-center bg-white rounded-lg p-8">
                    <h2 className="text-4xl text-gray-900 font-medium">{state === "login" ? "Login" : "Sign up"}</h2>
                    <p className="text-sm text-gray-500/90 mt-3">Welcome back! Please {state === "login" ? "Login" : "Sign up"} to continue</p>
                    ...
                    <p onClick={() => setState(prev => prev === "login" ? "register" : "login")} className="text-gray-500 text-sm mt-3 mb-11">{state === "login" ? "Don't have an account?" : "Already have an account?"} <Link to={state !== "login" ? '/login?state=login' : '/login?state=SignUp'} className="text-purple-500 hover:underline">click here</Link></p>
                </form>
            </div>
        </div>
    )
